...
# Corridor-flow targeting
# Large fog arcs are visibility boundaries, not movement directions. Estimate the
# local corridor axis around the player and prefer frontier goals that continue
# along that axis instead of drifting to the side of a fog arc.
USE_CORRIDOR_FLOW = True
FLOW_RADIUS = 34
FLOW_MIN_RADIUS = 7
FLOW_MIN_POINTS = 20
FLOW_ALIGNMENT_BONUS = 85.0
FLOW_SIDE_PENALTY = 42.0
FLOW_GOAL_DISTANCE_WEIGHT = 0.55
...
def unit_vector_from_points(cx, cy, tx, ty):
    dx = tx - cx
    dy = ty - cy
    length = math.hypot(dx, dy)
    if length < 1e-6:
...
def estimate_corridor_axis(reachable, clearance, cx, cy):
    """
    Estimate the local hallway axis around the player using PCA over safe,
    reachable hallway pixels. The result is directionless: (vx, vy) and
    (-vx, -vy) are the same corridor. This avoids treating a wide fog arc as
    a movement direction and keeps goals centered through necks/corridors.
    """
    if not USE_CORRIDOR_FLOW:
        return None

    safe = ((reachable > 0) & (clearance >= max(2.0, MIN_CLEARANCE - 1.0)))
    ys, xs = np.where(safe)
    if len(xs) < FLOW_MIN_POINTS:
        ys, xs = np.where(reachable > 0)
        if len(xs) < FLOW_MIN_POINTS:
            return None
    ...
    for x, y in zip(xs, ys):
        ...
        dy = float(y - cy)
        d = math.hypot(dx, dy)
        if FLOW_MIN_RADIUS <= d <= FLOW_RADIUS:
            pts.append([dx, dy])
            # Centerline pixels get more vote, so the axis follows hallway flow
            # instead of wall outlines or bulky rooms.
            weights.append(max(float(clearance[y, x]), 1.0))

    if len(pts) < FLOW_MIN_POINTS:
        return None
    ...
    mean = np.sum(pts * weights[:, None], axis=0)
    centered = pts - mean
...
def corridor_flow_score(axis, cx, cy, tx, ty):
    """
    Lower is better. Rewards targets lying along the local corridor axis and
    penalizes side drift. Since the axis is directionless, abs(dot) is used.
    """
    if axis is None:
        return 0.0

    ux, uy = unit_vector_from_points(cx, cy, tx, ty)
    if ux == 0.0 and uy == 0.0:
        return 0.0

    ax, ay = axis
    alignment = abs(ux * ax + uy * ay)          # 1 = along corridor, 0 = sideways
    side = 1.0 - alignment
    dist = math.hypot(tx - cx, ty - cy)

    return (
        -alignment * FLOW_ALIGNMENT_BONUS
        + side * FLOW_SIDE_PENALTY
        + side * dist * FLOW_GOAL_DISTANCE_WEIGHT
    )
...
def make_fog_frontiers(fog, reachable, clearance, cx, cy):
    ...
    """
    corridor_axis = estimate_corridor_axis(reachable, clearance, cx, cy)
    ...
    for i in range(1, num_labels):
        ...
        for px, py in zip(gx, gy):
            ...
            flow_penalty = corridor_flow_score(corridor_axis, cx, cy, px, py)
        ...
        frontiers.append({
            ...
            "flow_axis": corridor_axis,
            "flow_score": corridor_flow_score(corridor_axis, cx, cy, best_goal[0], best_goal[1]),
        })
...
def choose_reachable_goal(fog, reachable, clearance, cx, cy):
    ...
    for f in frontiers:
        ...
        # Flow score prevents wide fog arcs from pulling the goal sideways.
        # It is separate from priority: priority picks the branch; flow picks the
        # best hallway-centered point on that branch.
        flow_penalty = float(f.get("flow_score", 0.0))
